...
def esios_api(id_code, fecha_inicial, fecha_final, esios_token):
    try:
        #podemos cambiar el time_agg por sum
        address = "http://api.esios.ree.es/indicators/%s?start_date=%sT21:00:00Z&end_date=%sT23:00:00Z&time_trunc=day&time_agg=average" % (id_code, fecha_inicial, fecha_final)
...
codigo = 602
data = esios_api(codigo, "2023-12-01", "2024-01-05", esios_token)

print(data)


fig = px.bar(data, x='fecha', y='valor')
fig.update_layout({
